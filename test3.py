...
from data import data_generate_exp, data_generate_non_adp
from algorithm import Gbandits, Gbandits_adaptive, Gbandits_both

def regret_comp(index='0'):
    regret_matrix = [[0.0] * 1258] * 7
    std_matrix = [[0.0] * 1258] * 7
    if index == '1':
        rewardVector, sum_reward = data_generate_non_adp(10)
    else:
...
def test(index):
    labels = ['UMAB-G', 'UMAB-G-A', 'UMAB-G-B']
